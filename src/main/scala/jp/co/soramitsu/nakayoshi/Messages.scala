...
case class MsgFromTelegram(chatId: ChatTg, msgId: MsgTg, user: String, alias: Option[String],
                           msg: Option[(String, Seq[MessageEntity])],
                           fileUrl: Option[String], fwd: Option[String]) extends MsgFrom {
  ...
  override def toGitter(id: ChatGt)(implicit hs: HandlerSettings): Option[Any] = if (msg.isDefined || fileUrl.isDefined) {
  ...
  override def toRocketchat(id: ChatRc)(implicit hs: HandlerSettings): Option[Any] = if (msg.isDefined || fileUrl.isDefined) {
    val userfill = alias.fold(user)(alias => s"[$user](https://t.me/$alias)")
...
case class MsgFromGitter(chatId: ChatGt, msgId: MsgGt, userId: String, username: String,
                         userUrl: String, content: String) extends MsgFrom {
  ...
  override def toRocketchat(id: ChatRc)(implicit hs: HandlerSettings): Option[Any] = Some(MsgSendGitter(id,
    s"*[$username](https://gitter.im$userUrl)* _via gitter_\n" +
