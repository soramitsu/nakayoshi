...
case class ConnectedMessage(
    telegramChat: Option[ChatTg],
    telegramMsg: Option[MsgTg],
    gitterChat: Option[ChatGt],
    gitterMsg: Option[MsgGt],
    rocketchatChat: Option[ChatRc],
    rocketchatMessage: Option[MsgRc]
) {
...
object ConnectedMessage {
  def create(
      telegram: Option[(ChatTg, MsgTg)] = None,
      gitter: Option[(ChatGt, MsgGt)] = None,
      rocketchat: Option[(ChatRc, MsgRc)] = None
  ): ConnectedMessage =
    apply(
      telegram.map(_._1),
      telegram.map(_._2),
